...
export const attachmentUrl = <F extends string>(
	filename: F,
): AttachmentUrl<F> => `attachment://${filename}`;

export const fileFieldName = <N extends number>(index: N): FileFieldName<N> =>
	`files[${index}]`;

export const createAttachmentPayload = (
	id: number,
	filename: string,
	description?: string,
): AttachmentPayload => ({
...
export const createFileAttachment = (
	...
	name: fileFieldName(index),
	filename,
	data,
	contentType,
});

export const isValidEmbedImageFilename = (filename: string): boolean => {
	const ext = filename.split(".").pop()?.toLowerCase();
	return (
		ext !== undefined &&
		Object.values(EmbedImageFormat).includes(ext as EmbedImageFormat)
	);
};
